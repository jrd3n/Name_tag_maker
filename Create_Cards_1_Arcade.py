...
if __name__ == "__main__":
    ...
    first_name_font = "ArcadeClassic"
    last_name_font = "Arcade Interlaced"

    first_name_sizes = [150, 150, 150, 130, 105, 85, 70, 65]
    last_name_sizes = [100, 100, 90, 75, 60, 50, 40, 35, 33, 30]

    for first_name, last_name in Names.items():
        print(f"Processing {first_name}, {last_name}")

        try:
            first_name_text_size = first_name_sizes[min(len(first_name)-1, len(first_name_sizes)-1)]
            last_name_text_size = last_name_sizes[min(len(last_name)-1, len(last_name_sizes)-1)]
